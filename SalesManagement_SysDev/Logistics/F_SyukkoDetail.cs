...
{
    public partial class F_SyukkoDetail : Form
    {
        public int syID;
        public int orID;
        public int status;
        ...
        private List<T_SyukkoDetailDsp> SyukkoDetail;
        private List<T_SyukkoDetailDsp> subSyukkoDetail;
        ...
        public F_SyukkoDetail()
        {
            InitializeComponent();
        }

        private void F_SyukkoDetail_Load(object sender, EventArgs e)
        {
            textBoxSyukkoId.Text = syID.ToString();
        ...
        private void GetComboBoxData()
        {
            try
            {
                cmbSyukkoDetailId.DisplayMember = "SyDetailID";
                cmbSyukkoDetailId.ValueMember = "SyDetailID";
                cmbSyukkoDetailId.DataSource = SyukkoDetail;
                cmbSyukkoDetailId.SelectedIndex = -1;
        ...
        private void RefreshSyukkoDetailCombo()
        {
            try
            {
                int temp = 0;
                if (cmbSyukkoDetailId.SelectedValue != null)
                {
                    temp = int.Parse(cmbSyukkoDetailId.SelectedValue.ToString());
                }
                cmbSyukkoDetailId.DataSource = subSyukkoDetail;
                cmbSyukkoDetailId.SelectedIndex = subSyukkoDetail.FindIndex(x => x.SyDetailID == temp);
            }
            catch
            {
                throw;
        ...
        private void btnCon_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int SyID = int.Parse(textBoxSyukkoId.Text);
                var syukkoData = syukko.GetSyukkoData(new T_SyukkoRead { SyID = SyID, SyFlag = -1, SyStateFlag = -1 }).Single();
                ...
                DialogResult dialog = MessageBox.Show(string.Format("出庫ID：{0}を確定しますか。", SyID), "確認", MessageBoxButtons.OKCancel);
                if (dialog != DialogResult.OK)
                {
                    return;
                }
                syukko.FinalizeSyukkoData(arrivalData, arrivalDetailData, logEmID);
                updatedSyukkoData = syukko.GetSyukkoData(new T_SyukkoRead { SyID = SyID, SyFlag = -1, SyStateFlag = -1 }).Single();
                status = updatedSyukkoData.SyStateFlag;
                textBoxState.Text = statusString[status];
                if (status != 0)
                {
                    radioBtnCon.Enabled = false;
        ...
        private bool GetValidDataAtConfirm(T_SyukkoDsp syukkoData)
        {
            try
            {
                ...
                {
                    MessageBox.Show("本出庫IDは確定済です。", "入力エラー");
                    status = 1;
                    textBoxState.Text = statusString[status];
                    return false;
                }
                ...
                {
                    MessageBox.Show("本出庫に対応する受注が削除済です。");
                    orFlag = 2;
                    textBoxOrderId.BackColor = SystemColors.AppWorkspace;
        ...
        private void GenerateDataAtSelect()
        {
            ...
            {
                int sydetailID = 0;
                if (cmbSyukkoDetailId.SelectedIndex != -1)
                {
                    sydetailID = (int)cmbSyukkoDetailId.SelectedValue;
                }
                int prID = 0;
                ...
                // データの抽出
                SyukkoDetail = syukko.GetSyukkoDetailData(selectCondition);
                subSyukkoDetail = new List<T_SyukkoDetailDsp>(SyukkoDetail);
                subSyukkoDetail.Insert(0, new T_SyukkoDetailDsp { SyDetailID = null });
            }
            catch
            {
                throw;
        ...
        private void SetSelectDate()
        {
            try
            {
                ...
                int pageNo = 0;
                dataGridViewDsp.DataSource = SyukkoDetail.Skip(pageSize * pageNo).Take(pageSize).ToList();
                int dataCount=SyukkoDetail.Count;
                //dataGridViewの総ページ数
        ...
        private void radioBtnCon_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbSyukkoDetailId.Enabled = false; //詳細ID
        ...
        private void radioBtnSea_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                AllClear();
                label1.Enabled = true;
                cmbSyukkoDetailId.Enabled = true;  //詳細ID
        ...
        private void btnFirstPage_Click(object sender, EventArgs e)
        {
            try
            {
                dataGridViewDsp.DataSource = SyukkoDetail.Take(pageSize).ToList();
                dataGridViewDsp.Refresh();
                //ページ番号の設定
                textBoxPageNo.Text = "1";
            }
        ...
        private void btnNextPage_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int lastNo = (int)Math.Ceiling(SyukkoDetail.Count / (double)pageSize);
                if (pageNo < lastNo)
                {
                    dataGridViewDsp.DataSource = SyukkoDetail.Skip(pageSize * pageNo).Take(pageSize).ToList();
                    dataGridViewDsp.Refresh();
