...
namespace SalesManagement_SysDev
{
    class SyukkoDbConnection
    {
            ...
            try
            {
                using (var context = new SalesManagement_DevContext())
                {
                    var tb = from Syukko in context.T_Syukkos
                             join Order in context.T_Orders on Syukko.OrID equals Order.OrID
                             ...
                             group new { Syukko, subEmployee, Client, SalesOfficce,Order } by new { Syukko.SyID } into g
            ...
            try
            {
                using (var context = new SalesManagement_DevContext())
                {
                    var tb = from Syukko in context.T_Syukkos
                             ...
                             where (selectCondition.SyFlag == -1 || Syukko.SyFlag == selectCondition.SyFlag) &&
                             (selectCondition.SyID == 0 || Syukko.SyID == selectCondition.SyID) &&
                             (selectCondition.EmID == 0 ||(selectCondition.EmID==-1&&subEmployee.EmFlag!=0)|| selectCondition.EmID == Syukko.EmID) &&
                             (selectCondition.ClID == 0 ||(selectCondition.ClID==-1&&Client.ClFlag!=0)|| selectCondition.ClID == Syukko.ClID) &&
                             (selectCondition.SoID == 0 ||(selectCondition.SoID==-1&&SalesOfficce.SoFlag!=0)|| selectCondition.SoID == Syukko.SoID) &&
                             (selectCondition.OrID == 0 ||(selectCondition.OrID==-1&&Order.OrFlag!=0)|| selectCondition.OrID == Syukko.OrID) &&
                             ...
                             (selectCondition.PrID == 0 ||(selectCondition.PrID==-1&&subProduct.PrFlag!=0)|| selectCondition.PrID == subSyukkoDet.PrID) &&
            ...
            try
            {
                using (var context = new SalesManagement_DevContext())
                {
                    var tb = from SyukkoDet in context.T_SyukkoDetails
                             ...
                             (selectCondition.PrID == 0 ||(selectCondition.PrID==-1&&Product.PrFlag!=0)|| SyukkoDet.PrID == selectCondition.PrID) &&
        ...
        public void FinalizeSyukkoData(T_Arrival regData, List<T_ArrivalDetail> regDetailData, int EmID)
        {
            try
            {
                using (var context = new SalesManagement_DevContext())
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        ...
                        if (regData.ArID > NumericRange.ID)
                        {
                            ...
                            context.Database.ExecuteSqlCommand("DBCC CHECKIDENT('T_Arrival',RESEED,0)");
                            context.Database.ExecuteSqlCommand("DBCC CHECKIDENT('T_Arrival')");
                            throw new Exception("入荷IDが既定の数字を超えています。");
                        }
                        foreach (var row in regDetailData)
                        {
                            row.ArID = regData.ArID;
                            context.T_ArrivalDetails.Add(row);
                            context.SaveChanges();
                            if (row.ArDetailID > NumericRange.ID)
                            {
                                transaction.Rollback();
                                context.Database.ExecuteSqlCommand("DBCC CHECKIDENT('T_ArrivalDetail',RESEED,0)");
                                context.Database.ExecuteSqlCommand("DBCC CHECKIDENT('T_ArrivalDetail')");
        ...
        public bool CheckSyukkoIsInsertable(int Syid)
        {
            try
            {
                using(var context=new SalesManagement_DevContext())
