...
namespace SalesManagement_SysDev
{
    public partial class F_Arrival : Form
    {
        ...
        private List<T_ArrivalDsp> Arrival;
        private List<T_ArrivalDsp> subArrival;
        ...
        private void GetComboBoxData()
        {
            ...
            {
                //注文IDのコンボボックスData読込
                cmbArrivalId.DisplayMember = "ArID";
                cmbArrivalId.ValueMember = "ArID";
                cmbArrivalId.DataSource = Arrival;
                cmbArrivalId.SelectedIndex = -1;
        ...
        private void btnCon_Click(object sender, EventArgs e)
        {
            try
            {
                if (!InputCheckAtConfirm())
                    return;
                int ArId = int.Parse(cmbArrivalId.SelectedValue.ToString());
                var arrivalData = Arrival.Single(x => x.ArID == ArId);
                ...
                if (!GetValidDataAtConfirm(arrivalData))
                {
                    ...
                    arrivalData.ArStateFlag = updatedArrivalData.ArStateFlag;
                    arrivalData.ArBoolStateFlag = updatedArrivalData.ArBoolStateFlag;
                    arrivalData.EmID = updatedArrivalData.EmID;
                    arrivalData.EmName = updatedArrivalData.EmName;
                    arrivalData.ArDate = updatedArrivalData.ArDate;
                    dataGridViewDsp.Refresh();
                    checkBoxFlag.Checked = updatedArrivalData.ArBoolFlag;
                    if (updatedArrivalData.ArHidden == null)
                    {
                        textBoxHideRea.Text = "";
                    }
                    cmbEmployeeId.SelectedIndex = Employee.FindIndex(x => x.EmID == updatedArrivalData.EmID);
                    cmbState.SelectedIndex = State.FindIndex(x => x.itemValue == updatedArrivalData.ArStateFlag);
                    if (updatedArrivalData.ArStateFlag != 0)
                    {
                        dateTimePickerS.Format = DateTimePickerFormat.Long;
                        dateTimePickerS.Text = updatedArrivalData.ArDate.ToString();
                ...
                DialogResult dialog = MessageBox.Show(string.Format("入荷ID：{0}を確定しますか。", arrivalData.ArID), "確認", MessageBoxButtons.OKCancel);
                if (dialog != DialogResult.OK)
                {
                    return;
                }
                //確定処理
                arrival.FinalizeArrivalData(shipmentData, shipmentDetailData, LoginEmID);
                updatedArrivalData = arrival.GetArrivalData(new T_ArrivalRead { ArID = ArId, ArFlag = -1, ArStateFlag = -1 }).Single();
                arrivalData.ArStateFlag = updatedArrivalData.ArStateFlag;
                arrivalData.ArBoolStateFlag = updatedArrivalData.ArBoolStateFlag;
                arrivalData.EmID = updatedArrivalData.EmID;
        ...
        private bool InputCheckAtConfirm()
        {
            try
            {
                if (cmbArrivalId.SelectedIndex == -1)
                {
                    MessageBox.Show("入荷IDを選択してください。", "選択エラー");
                    cmbArrivalId.Focus();
                    return false;
                }
                return true;
        ...
        private bool GetValidDataAtConfirm(T_ArrivalDsp arrivalData)
        {
            try
            {
                ...
                {
                    MessageBox.Show("選択した入荷IDは確定済です。", "選択エラー");
                    cmbArrivalId.Focus();
                    return false;
                }
                if (!order.CheckOrderIsActive(arrivalData.OrID))
                {
                    MessageBox.Show("選択した入荷に対応する受注は削除されています。", "選択エラー");
                    cmbArrivalId.Focus();
                    return false;
                }
                return true;
        ...
        private List<T_ShipmentDetail> GenerateDetailDataAtConfirm(List<T_ArrivalDetailDsp> arrivalDetailData)
        {
            try
            {
                ...
                foreach (var x in arrivalDetailData)
                {
                    shipmentDetailData.Add(new T_ShipmentDetail
                    {
                        PrID = x.PrID,
                        ShQuantity = x.ArQuantity
                    });
                }
        ...
        private void btnDetail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                f_Detail = new F_ArrivalDetail();
                int arID = (int)cmbArrivalId.SelectedValue;
                f_Detail.arID = arID;
                ...
                f_Detail.ShowDialog();
                var arrivalData = Arrival.Single(x => x.ArID == arID);
                var updatedArrivalData = arrival.GetArrivalData(new T_ArrivalRead { ArID = arID, ArFlag = -1, ArStateFlag = -1 }).Single();
                arrivalData.ArStateFlag = updatedArrivalData.ArStateFlag;
                arrivalData.ArBoolStateFlag = updatedArrivalData.ArBoolStateFlag;
                arrivalData.EmID = updatedArrivalData.EmID;
                arrivalData.EmName = updatedArrivalData.EmName;
                ...
                cmbEmployeeId.SelectedIndex = Employee.FindIndex(x => x.EmID == updatedArrivalData.EmID);
                cmbState.SelectedIndex = State.FindIndex(x => x.itemValue == arrivalData.ArStateFlag);
                if (updatedArrivalData.ArStateFlag != 0)
                {
                    dateTimePickerS.Format = DateTimePickerFormat.Long;
                    dateTimePickerS.Text = updatedArrivalData.ArDate.ToString();
        ...
        private void GenerateDataAtSelect(bool selectFlg)
        {
            ...
            {
                int arID = 0;
                if (cmbArrivalId.Text != "")
                {
                    arID = int.Parse(cmbArrivalId.Text);
                }
                int orID = 0;
                if (!string.IsNullOrEmpty(textBoxOrderId.Text.Trim()))
                {
                ...
                {
                    // 検索条件のセット
                    T_ArrivalRead selectCondition = new T_ArrivalRead()
                    {
                        ArID = arID,
                        OrID = orID,
                        SoID = soID,
                        ...
                        ScID = scID,
                        ArFlag = hideFlg,
                        ArHidden = textBoxHideRea.Text.Trim()
                    };
                    // データの抽出
                    Arrival = arrival.GetArrivalData(selectCondition);
                    subArrival = new List<T_ArrivalDsp>(Arrival);
                    subArrival.Insert(0, new T_ArrivalDsp { ArID = null });
                }
                else
                {
                    ...
                    // データの抽出
                    Arrival = arrival.GetArrivalData(selectCondition, sdate, edate);
                    subArrival = new List<T_ArrivalDsp>(Arrival);
                    subArrival.Insert(0, new T_ArrivalDsp { ArID = null });
                }
            }
        ...
        private void btnHidden_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var arrivalData = Arrival.Single(x => x.ArID == arID);
                if (iFlg == 2 && arrivalData.ArFlag == 0 && order.CheckOrderIsActive(int.Parse(textBoxOrderId.Text)))
                {
                    DialogResult dialog = MessageBox.Show(string.Format("入荷ID：{0}は確定されておらず、対応する受注ID：{1}は削除されていませんがよろしいですか。", arrivalData.ArID, arrivalData.OrID), "確認", MessageBoxButtons.OKCancel);
                    if (dialog != DialogResult.OK)
                    {
                        return;
                    }
                }
                string arhidden = textBoxHideRea.Text.Trim() == string.Empty ? null : textBoxHideRea.Text.Trim();
                arrival.UpdateArrivalHidden(arID, iFlg, arhidden);
                GetAllData();
                AllClear();
            }
            catch (Exception ex)
        ...
        private void RefreshArrivalCombo()
        {
            try
            {
                int temp = 0;
                if (cmbArrivalId.SelectedValue != null)
                {
                    temp = int.Parse(cmbArrivalId.SelectedValue.ToString());
                }
                if (radioBtnSea.Checked)
                {
                    cmbArrivalId.DataSource = subArrival;
                    cmbArrivalId.SelectedIndex = subArrival.FindIndex(x => x.ArID == temp);
                }
                else
                {
                    cmbArrivalId.DataSource = Arrival;
                    cmbArrivalId.SelectedIndex = Arrival.FindIndex(x => x.ArID == temp);
                }
            }
            catch
            {
        ...
        private bool GetAllData()
        {
            ...
            {
                //全件取得
                Arrival = arrival.GetArrivalData(new T_ArrivalRead { SoID = (int)cmbSalesOfficeId.SelectedValue, ArStateFlag = -1 });
                subArrival = new List<T_ArrivalDsp>(Arrival);
                subArrival.Insert(0, new T_ArrivalDsp { ArID = null });
                if (Arrival == null)
                    return false;
        ...
        private void dataGridViewDsp_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (updFlg == true)
                {
                    ...
                    cmbArrivalId.Text = dataGridViewDsp.Rows[dataGridViewDsp.CurrentRow.Index].Cells[0].Value.ToString();
        ...
        private void cmbArrivalId_SelectionChangeCommitted(object sender, EventArgs e)
        {
            try
            {
                if (updFlg == true)
                {
                    ...
                    {
                        int index = Arrival.FindIndex(x => x.ArID == int.Parse(cmbArrivalId.SelectedValue.ToString()));
                        textBoxOrderId.Text = Arrival[index].OrID.ToString();
                        cmbClientId.SelectedIndex = Client.FindIndex(x => x.ClID == Arrival[index].ClID);
                        if (Arrival[index].ArStateFlag != 0)
                        {
                            cmbEmployeeId.SelectedIndex = Employee.FindIndex(x => x.EmID == Arrival[index].EmID);
                            dateTimePickerS.Format = DateTimePickerFormat.Long;
                            dateTimePickerS.Text = Arrival[index].ArDate.ToString();
                            dateTimePickerS.Checked = true;
                        }
                        else
                        {
                        ...
                        checkBoxFlag.Checked = Arrival[index].ArBoolFlag;
                        if (Arrival[index].ArHidden == null)
                        {
                            textBoxHideRea.Text = "";
                        }
                        else
                        {
                            textBoxHideRea.Text = Arrival[index].ArHidden.ToString();
                        }
                        cmbState.SelectedValue = Arrival[index].ArStateFlag;
                        if (radioBtnCon.Checked)
                        {
                            if (Arrival[index].ArStateFlag == 0 && Arrival[index].OrderFlag == 0)
                            {
                                btnCon.Enabled = true;
                            }
                            else
